...
router.post("/register", async (req, res) => {
  ...
  const existing = db.prepare("SELECT id FROM developers WHERE username = ?").get(username.trim());
  if (existing) {
  ...
  try {
    ...
    const info = db
      .prepare("INSERT INTO developers (username, password_hash) VALUES (?, ?)")
      .run(username.trim(), passwordHash);

    return res.status(201).json({
      message: "Compte développeur créé avec succès.",
      developer: { id: info.lastInsertRowid, username: username.trim() },
    });
  } catch (err) {
    console.error(err);
...
router.post("/login", async (req, res) => {
  ...
  const developer = db.prepare("SELECT * FROM developers WHERE username = ?").get(username.trim());
  if (!developer) {
